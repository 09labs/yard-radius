...
static char rcsid[] = "$Id: proxy.c,v 1.5 2002/03/05 19:17:57 kiavik Exp $";
...
void rad_proxy(int fd)
{
	...
	__debug("proxy server %s/%d.%d replied with code=%d, length=%d\n", ipaddr2strp(host), port, authreq->id, authreq->code, result);
	if (debug_flag > 2) {
	...
	if (server->flags & PEER_NOPROXY) { /* server can't do proxy */
		if (qp == (AUTH_REQ *)NULL) {	/* not found in queue */
			__debug("rad_proxy: response from %s/%d.%d not matched in forwarded queue, dropped\n", ipaddr2strp(host), port, authreq->id);
			reqfree(authreq, "rad_proxy");
			return;
		} else {
			qpop = qp;
	...
	if (qp == (AUTH_REQ *)NULL) {
		if (authreq->code == PW_ACCOUNTING_RESPONSE) {
			 ...
			 */
			__debug("rad_proxy: accounting-response from %s/%d.%d not matched in forwarded queue, dropped\n",
				ipaddr2strp(host), port, authreq->id);
			reqfree(authreq, "rad_proxy");
...
void send_proxy2client(int fd,AUTH_REQ*authreq,PEER*server,AUTH_REQ*qp)
{
	...
	__debug("forwarding reply code %d from %s to %s\n",
...
int handle_proxy(AUTH_REQ*authreq)
{
	...
	if (authreq->code != PW_AUTHENTICATION_REQUEST &&
		...
		__debug("handle_proxy called for packet type %d unexpectedly\n",
			authreq->code);
...
void push_proxy(AUTH_REQ*authreq)
{
	...
	/* Proxy-State contains: Timestamp, Client, Port, Id in network order,
 	...
 	if((attr = dict_attrget(PW_PROXY)) == (DICT_ATTR *)NULL) {
		__debug("add Proxy (%d) to dictionary\n", PW_PROXY);
...
void send_proxy2server(AUTH_REQ*authreq,PEER*server)
{
	...
	__debug("forwarding request from %s to %s/%d.%d for %s\n",
...
u_char getnextid(AUTH_REQ *authreq)
{
	...
	while (qp != (AUTH_REQ *)NULL) {
		if (qp->timestamp + max_proxy_time < now) {
			if (++flushcount % 100 == 0) {
				__debug("%d proxy requests expired unanswered\n",flushcount);
			}
