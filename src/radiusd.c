...
static char rcsid[] = "$Id: radiusd.c,v 1.9 2002/03/05 19:17:57 kiavik Exp $";
...
int main( int argc,char **argv )
{
  ...
  __debug("initializing dictionary\n");
  /* Initialize the dictionary */
  if(dict_init() != 0) { rad_exit(-1); }
  dict_dump();

  __debug("initializing configuration values\n");
   ...
   */
  __debug("Disconnecting from session\n");
  if(debug_flag == 0) {
  	pid = fork();
  	if(pid < 0) {
   ...
   *	Disconnect from tty
   */
  __debug("Disconnecting from tty\n");
  ...
  update_proxy();
         ...
         if (vports_flag == 1 && spawn_flag == 1) {
                 spawn_flag = 0;
                 __debug("virtual ports disable spawning\n");
         }
#endif /* VPORTS */
...
int open_udpsock(u_short *port,int defport,char *service)
{
  ...
  __debug("using udp port %d for %s\n", *port,service);
...
void send_packet(int fd, UINT4 ipaddr, u_short port, char * buffer, int length)
{
  ...
  __debug("message sent to %s/%d.%d code=%d, length=%d\n",
...
void rad_request(int fd)
{
  ...
  if ((pair = get_attribute(authreq->request,PW_STATE)) != (VALUE_PAIR *)NULL) {
  	buf = pair->strvalue;
  	__debug("rad_request: PW_STATE<%s>\n", buf);
#ifdef SECURID
  	/*
  ...
  __debug("rad_request: challenge_response from %s for child %d\n",
...
AUTH_REQ *radrecv(UINT4	host,u_short udp_port,char* secret,u_char* buffer,int length)
{
  ...
  __debug("message received from %s/%d.%d code=%d, length=%d\n",
  	...
  	while(result == 0 && check_item != (VALUE_PAIR *)NULL) {
  		...
  		case PW_EXPIRATION:
 			...
 			if (!strncasecmp(check_item->strvalue,"SHADOW", 6))
  			...
  			if(retval < 0) {
  				result = -2;
  				snprintf(umsg,(size_t)(AUTH_STRING_LEN+2),"Password Has Expired\r\n");
  				user_msg = umsg;
...
**/
		case PW_CONNECT_RATE:
        	...
        	case PW_LOGINS:
                ...
                case PW_MAXDTIME:
                     ...
                     if (retval != 0) {
                         result = -2;
                         sprintf(umsg, "Total on-line daily time expired (%d hours)\r\n",(unsigned int)check_item->lvalue);
                         user_msg = umsg;
                 ...
                 case PW_MAXMTIME:
                     ...
                     if (retval != 0) {
                         result = -2;
                         sprintf(umsg, "Total on-line time expired (%d hours)\r\n", (unsigned int)check_item->lvalue);
                         user_msg = umsg;
                 ...
                 case PW_MAXYTIME:
                     ...
                     if (retval != 0) {
                         result = -2;
                         sprintf(umsg, "Total on-line yearly time expired (%d hours)\r\n", (unsigned int)check_item->lvalue);
                         user_msg = umsg;
                 ...
                 case PW_TIME:
                     ...
                     if (retval != 0) {
                         result = -2;
                         sprintf(umsg, "Not allowed to login at this time\r\n");
                         user_msg = umsg;
                 ...
                 case PW_MAXDTRAFFIC:
                     ...
                     if (retval != 0) {
                         result = -2;
                         sprintf(umsg, "Maximum allowed daily traffic size reached (%d KB)\r\n",(unsigned int)check_item->lvalue);
                         user_msg=umsg;
                 ...
                 case PW_MAXMTRAFFIC:
                     ...
                     if (retval != 0) {
                         result = -2;
                         sprintf(umsg, "Maximum allowed monthly traffic size reached (%d KB)\r\n", (unsigned int)check_item->lvalue);
                         user_msg=umsg;
                 ...
                 case PW_MAXYTRAFFIC:
                     ...
                     if (retval != 0) {
                         result = -2;
                         sprintf(umsg, "Maximum allowed yearly traffic size reached (%d KB)\r\n", (unsigned int)check_item->lvalue);
                         user_msg=umsg;
  ...
  if(result == 0) {
  	...
  	case PW_AUTHTYPE_NONE:
  		...
  		__debug("entry for user \"%s\" has no Password or Auth-Type check-item\n",authreq->name);
  		result = 0;
  		break;
...
void send_challenge(AUTH_REQ *authreq,char *msg,char *state,int activefd)
{
  ...
  __debug("sending challenge to %s\n", req2strp(authreq));
...
int build_packet(AUTH_REQ*authreq,VALUE_PAIR*reply,char*msg,u_char code,
  	 int forward,u_char*buffer,size_t buflen )
{
  AUTH_HDR		*auth;
  VALUE_PAIR		*item;
...
int update_clients(void)
{
  ...
  fclose(clientfd);
     	...
     	if (rename(newcache,oldcache) != 0) {
             ...
             return(-1);
     	} else { __debug("updated client cache with %d clients\n",nclients); }
