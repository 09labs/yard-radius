...
static char rcsid[] = "$Id: extens.c,v 1.9 2004/07/01 13:57:55 flovergine Exp $";
 ...
 *
 *   Corrected a lot of memory leaks - <francesco@yardradius.org>
 */ 
...
static 
int listed_on_file(const char *filename, const char *username)
{
    FILE *fp;
    char buffer[1024];
    char value[1024];
...
int allow_user(const char *username)
{
    char file_name[PATH_MAX];
    char buffer[1024];
 ...
 */
static int build_radlast_from_authreq(radlast *rl, AUTH_REQ *authreq)
{
    int		status_type = 0;
    time_t	crt_time;
    VALUE_PAIR	*pair;
 ...
 */
static int update_db_record(int action, datum *content, radlast *rl,
                            int month, int day)
{	
    void *data;
    int datasize;
 ...
 */
static 
int insert_db_record(int action,datum *content,radlast *rl,int month,int day)
{
    user_entry *tmp;
...
static int update_radlast(radlast *rl)
{
    pid_t 	child_pid;
...
int update_user_status(AUTH_REQ *authreq)
{
    ...
    __debug("acct: %s user='%s', port=%d, time=%d, input=%d, output=%d\n",
...
int shadow_expired(const char *user)
{
	...
	__debug("shadow_expired: called with null argument\n");
	/* but the account is not expired... */
	return 0;
    }
    __debug("shadow_expired: checking shadow expiration for user '%s'\n", user);	
    setspent();
    spw = getspnam(user);
    if (spw == NULL) {
...
static int PAM_conv_acct(int num_msg, const struct pam_message **msg,
...
int unix_pam(const char *name, const char *passwd, const char *pamauth)
{
    ...
    __debug("unix_PAM: using pamauth string <%s> for pam.conf lookup\n", pamauth);    
    retval = pam_start(pamauth, name, &conv_acct, &pamh);
    if (retval == PAM_SUCCESS) {
	__debug("unix_PAM: function pam_start succeeded for <%s>\n", name);
	retval = pam_authenticate(pamh, 0);
    }
    if (retval == PAM_SUCCESS) {
	__debug("unix_PAM: function pam_authenticate succeeded for <%s>\n", name);
	retval = pam_acct_mgmt(pamh, 0);
    }

    /* do not remove block braces, it could break things due to __debug
     * macro */
    
    if (retval == PAM_SUCCESS) {
	__debug("unix_PAM: function pam_acct_mgmt succeeded for <%s>\n", name);
    }
    else {
	__debug("unix_PAM: PAM FAILED for <%s> failed\n", name);
    }
 ...
 *              session modules.
 */
static int PAM_conv_session(int num_msg, const struct pam_message **msg,
			    struct pam_response **resp, void *appdata_ptr)
{
...
void pam_session(AUTH_REQ *authreq)
{
	...
	__debug("about to init PAM session call for %s\n", username);    
	...
	/* start session */
	__debug("opening session for user %s\n", username);	
	...
	/* close session */
	__debug("closing session for user %s\n", username);
...
static acct_packet *in_acct_queue(acct_packet *packet, int type)
{
    acct_packet *this;
 ...
 */
static int add_acct_queue(acct_packet *packet)
{
 ...
 */
static int remove_acct_queue(acct_packet *packet)
{
 ...
 */
static int cleanup_acct_queue(void)
{
    acct_packet *temp;
...
auxdebug = fopen("nighthawk","a");
    ...
    if (time_str == NULL) {
	/* cry loud */
	__debug("allowed_times: passed a null pointer !!!\n");
	/* After all, it is okay... */
	return 0;
    }
    
    __debug("allowed_time: checking time against %s\n", time_str);
#ifdef EXT_DEBUG
Debug(auxdebug,"allowed_time: checking time against %s\n", time_str);
	 ...
	 */
	if (i == 0) {
	    __debug("allowed_time: time specification incompete - assuming Al; i=%d\n",i);
	    times[j].t_days = Mo_DAY|Tu_DAY|We_DAY|Th_DAY|Fr_DAY|Sa_DAY|Su_DAY;
	    beg_day = 0; /* Not a valid day to begin an interval */
	}
	...
	*/	
	if (cp[i]=='-') {
		...
		{
			__debug("Error in the token %d: cannot have such a day interval; skipped\n",j+1);
			error_tokens++;
		...
		*/
		if (end_day==0) {
			__debug("Error in the token %d: not a valid day\n",j);
			times[j].t_days = Mo_DAY|Tu_DAY|We_DAY|Th_DAY|Fr_DAY|Sa_DAY|Su_DAY;
	...
	*/
	...
	if (cp[i] && isdigit(cp[i]))
	{
		...
		__debug("Error in the token %d: too many digits for the begin time; skipped\n",j+1);
		/* 
	...
	{
	    __debug("Error in the token %d: begin time invalid; skipped\n",j+1);
	    error_tokens++;
	...
	if (cp[i] && isdigit(cp[i]))
	{
		 ...
		 */
		__debug("Error in the token %d: too many digits for the end time\n",j+1);
	...
	if (dtime > 2400 || dtime % 100 > 59)
	{
	    ...
	    */
	    __debug("Error in the token %d: end time invalid; skipped\n",j+1);
    ...
    if (error_tokens)
    	__debug(" %d error(s) found in the time string\n",error_tokens);
    if (session_timeout <= 0) {
    	__debug("allowed_time: access denied\n");
#ifdef EXT_DEBUG
...
fclose(auxdebug);
    ...
    __debug("allowed_time: access allowed for %d seconds\n",session_timeout);
#ifdef EXT_DEBUg
