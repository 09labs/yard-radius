...
static char rcsid[] = "$Id: radlast.c,v 1.8 2004/07/01 13:57:55 flovergine Exp $";
...
static void radlast_usage(void)
{
    ...
    printf("\t-n <num>\tPrint only <num> entries\n");
...
int main(int argc, char **argv)
{
    ...
    while ((flag = getopt(argc, argv, "a:F:H:cu:f:by:m:xhn:")) != EOF) {
	char *cp;
	switch (flag) {
	    case 'a':
		radacct_dir = optarg;
		...
		if (*cp) {
                   fatal_func("number of entries must be an integer\n");
                   radlast_usage();
		}
                break;
	    case 'y':
