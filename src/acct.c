...
static char rcsid[] = "$Id: acct.c,v 1.7 2002/03/05 19:17:57 kiavik Exp $";
...
void send_acct_reply( AUTH_REQ *authreq,VALUE_PAIR *reply,int activefd)
{
	...
	__debug("sending acct-response to %s\n", req2strp(authreq));
...
int calc_acctreq( UINT4	client,char *secret,int secretlen,u_char *buffer,int buflen ) 
{
	...
	memcpy(buffer+len,secret,buflen-len);
	md5_calc(digest, buffer, buflen);
